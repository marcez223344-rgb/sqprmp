...
**Datasets** are generated by deterministic TypeScript scripts (seeded PRNG) into a PGlite instance, then exported with `dumpDataDir()` to versioned snapshots (`<slug>-v<n>.tar.gz`). The same snapshot is served to the browser (static asset, cached) and loaded by the server engine (cached in memory per function instance).
...
- PGlite runs in a Web Worker (`@electric-sql/pglite/worker`) so the UI stays responsive; the worker is terminated and recreated on timeout.
- Snapshot cached in IndexedDB per dataset version; target 1–4 MB per dataset (row volumes are tuned for this).
