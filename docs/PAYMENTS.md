...
scholarship, a friend, a test account, or a bank transfer that had already landed in the owner's
account — it wrote the same row: an entitlement with `source = 'admin'`. The user directory and
`admin_metrics` classify that as **otorgado**, so a real sale never appeared as revenue and the
owner could not tell, a month later, which of his learners had paid him (owner feedback item 22).

Those are two different facts and the admin is now asked which one happened:

| The admin says      | What is written                                                                                                                                                                                                          | How it is reported            |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------- |
| **Pago recibido**   | an approved `purchases` row (provider `manual`, channel `bank_transfer`, amount and currency from the active price, the bank reference the admin typed) **and** an entitlement with `source = 'purchase'` pointing at it | **pagado**, counts as revenue |
| **Beca o cortesía** | only an entitlement with `source = 'admin'`                                                                                                                                                                              | **otorgado**, never revenue   |

Both go through `admin_grant_access(p_user_id, p_kind, …)` (service role only) and both write an
audit entry — `access.payment_recorded` or `access.comp_granted` — with the actor, the learner, the
reason and, for a payment, the reference and the amount.

**When the learner already opened a pending transfer**, `admin_grant_access` approves _that_ row
rather than creating a second one, so the reference code the learner was told to use on the
transfer is the one that ends up in the books. In other words there are two ways into the same
state and they converge:

- learner clicks "Ya pagué" on `/precios` → pending purchase `DM-XXXXXX` → admin approves it in
  `/admin/accesos`;
- the money simply arrives and there is no pending row → admin picks the learner and records the
  payment with the bank reference → purchase `TR-XXXXXX`, approved, plus the entitlement.

Refunds still run through `apply_payment_event`; a manually recorded purchase is revoked the same
way an admin revokes any entitlement, with a reason.

**Related admin behaviour settled in the same change**

- The learner is chosen from a **picker** (`admin_search_learners`, alias and display name only,
  never an email), not typed from memory.
- A scholarship code is **generated by default** from an alphabet with no `O`/`0` or `I`/`1`
  (`src/lib/payments/promo-code.ts`), because codes are dictated on calls and retyped from
  screenshots. A custom code is still allowed. Codes compare case-insensitively
  (migration `20260922140000`).
- "Días de acceso" is the only duration input in both the grant form and the scholarship form; the
  end date is **derived and shown live**. The scholarship form's other date now says what it really
  is — when the _code_ stops being redeemable — which is a different fact from how long the access
  it grants lasts.
- **No email is sent.** There is no transactional email provider in this project (see OA-25). A
  learner whose access becomes active is told in the app: a one-time notice at the top of the
  authenticated shell, dismissed via `acknowledge_entitlements()` (so the dismissal survives a new
  device), plus `/acceso`, which states what they have and until when.
- Exporting the user directory to CSV is admin-only, rate limited and audit-logged
