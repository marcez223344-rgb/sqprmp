...
| `tres-secciones`    | `Library`       | dominio    | II   | override: `Layers` is the intermediate level icon       |
...
**Profile / settings**: identity, avatar, alias (change limited), certificate name, privacy (leaderboard opt-in), goals, access status (plan, purchase date, invoice ref), export/delete account. A «Tus logros» card comes first: issued, non-revoked certificates with Verify and PDF links, and a `success` meter per level over published lessons.
