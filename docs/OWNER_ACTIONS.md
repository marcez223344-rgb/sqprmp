...
| OA-03 | **Transfer details** for `src/config/pricing.ts` → `manualTransferInstructions`: bank alias + CBU, Mercado Pago alias, Wallbit TAG.                                                                  | All three currently render the literal text `PENDIENTE-DE-CONFIGURAR` **to learners** on `/precios`. Manual transfer is the day-one channel. | DONE — Banco Galicia supplied 2026-09-23; Mercado Pago + Wallbit still missing and stay hidden |
