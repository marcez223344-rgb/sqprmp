...
- `RESEND_API_KEY` (owner email alerts, D-43) is optional and server-only; it is read only through `src/lib/env/server.ts` and never logged. Log lines from `src/lib/notifications/owner.ts` carry the event type and an HTTP status, never the key, the recipient or the email body.
...
| Exercise problem reports (note + SQL)    | Fixing exercise content (D-42). Private: readable by the reporter and by admins only (`/admin/reportes`), never shown to other learners; the reporter is identified by alias to the admin                                                                                                                                                                                                            | Account lifetime; cascade-deleted with the profile  |
| Owner email alerts (via Resend)          | Tell the owner at once about a new exercise report or a code redemption (D-43). **Resend (resend.com) is a processor** receiving, per event: the learner's alias, the exercise title/slug, report category and the note (cut to `limits.ownerNotifications.noteExcerptChars`), or the code, what it granted and its uses. Never SQL, email address or display name. Sent only to the owner's address | Resend's own log retention; the owner's mailbox     |
