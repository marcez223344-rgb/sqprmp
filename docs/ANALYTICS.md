...
Implementation (Phase 8): `src/lib/analytics/events.ts` holds the Zod schema per event (strict: unknown keys are rejected) and `src/lib/analytics/track.ts` writes rows with the admin client, fire-and-forget (failures are logged without properties and never break a learner flow). `page_viewed`, `signup_*`, `onboarding_step_completed`, `exercise_started`, `query_run` and `streak_frozen` are specified but not yet emitted (D-15); everything else is wired at the server call sites listed in ROADMAP Phase 8.

Owner dashboard queries (SQL views): signup conversion, onboarding completion, first exercise started/completed, free consumption, paywall→checkout→purchase, course completion, drop-off per section, hardest exercises (attempts, hint rate, reveal rate), frequent SQLSTATEs, weekly active learners, D1/D7/D30 retention. Implemented as the `admin_metrics(p_free_limit)` RPC rendered at `/admin/metricas` (computed on request; see DATABASE_DESIGN.md §4i).
