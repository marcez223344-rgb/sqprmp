...
- **pgTAP**: the twelve suites run in CI via `supabase test db` and locally via `npx supabase start` then `npx supabase test db`, both of which need Docker. **First executed on 2026-09-24** (Docker Desktop 29.8.0 on WSL 2, after the owner ran the elevated `wsl --install`): `Files=12, Tests=208, Result: PASS`, against a fresh database built from all 20 migrations and the three seeds. Until that date they had never run anywhere — not locally, not in CI — which meant the RLS policies, the reward ledger, quiz grading and certificate eligibility were unverified while already serving learners. Re-run them after any migration; a schema change without a green pgTAP run is not done.
