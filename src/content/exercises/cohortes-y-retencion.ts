...
export const exercises: ExerciseDef[] = [
  {
    ...
    tables_used: ["users"],
    scenario_md:
      "En **Ritmo**, el equipo de Crecimiento va a construir su primer tablero de cohortes. Antes de medir retención necesita el denominador: cuánta gente entró en cada mes de alta y qué peso tiene cada cohorte sobre el total de oyentes registrados.",
    business_question_md:
      "A partir de `users`, devuelve una fila por mes de alta: `cohorte` (primer día del mes de `signup_at`, calculado **en UTC**, como `date`), `usuarios` (oyentes registrados en ese mes) y `pct_del_total` (porcentaje que representa esa cohorte sobre el total de oyentes de la tabla, redondeado a 2 decimales). Ordena por `cohorte` ascendente.",
    learning_objective:
...
ORDER BY 1;`,
  ...
  },
  {
    ...
    scenario_md:
      "Producto sospecha que mucha gente se registra en Ritmo y nunca llega a escuchar nada. Antes de rediseñar el alta quiere saber, cohorte por cohorte, qué proporción reprodujo al menos una canción dentro de sus primeros siete días.",
    business_question_md:
      "Considera solo a los oyentes con `signup_at` anterior al `2025-09-01` (en UTC), para que todos hayan tenido siete días completos de observación. Devuelve `cohorte` (primer día del mes de `signup_at` en UTC, como `date`), `usuarios` (tamaño de la cohorte), `activados` (cuántos tienen al menos una reproducción en el intervalo que va desde su `signup_at` hasta `signup_at + 7 días`, sin incluir ese límite) y `tasa_activacion_pct` (porcentaje redondeado a 2 decimales). Ordena por `cohorte` ascendente.",
    learning_objective:
...
ORDER BY 1;`,
  ...
  },
  {
    ...
    tables_used: ["users", "plays"],
    scenario_md:
      "El comité de producto de Ritmo adoptó una métrica única de retención temprana: «volvió a escuchar entre el día 1 y el día 30 después del alta». Quieren la serie por cohorte mensual para ver si mejoró a lo largo del tiempo.",
    business_question_md:
      "Toma a los oyentes con `signup_at` anterior al `2025-08-01` (en UTC), que son los que ya completaron su ventana de 30 días. Una persona cuenta como retenida si tiene al menos una reproducción con `played_at` mayor o igual a `signup_at + 1 día` y menor a `signup_at + 31 días` (es decir, se excluye el primer día). Devuelve `cohorte` (primer día del mes de `signup_at` en UTC, como `date`), `usuarios`, `retenidos_30d` y `retencion_30d_pct` (redondeado a 2 decimales), ordenado por `cohorte` ascendente.",
    learning_objective:
...
ORDER BY 1;`,
  ...
  },
  {
    ...
    tables_used: ["users", "plays"],
    scenario_md:
      "Con las métricas puntuales resueltas, Dirección pide la tabla completa: la matriz de cohortes de Ritmo, para pegarla en el tablero y poder leer tanto la curva de vida de cada cohorte como la comparación entre cohortes en el mismo momento de vida.",
    business_question_md:
      "Para cada cohorte de alta y cada mes de vida con actividad, devuelve `cohorte` (primer día del mes de `signup_at` en UTC, como `date`), `mes_indice` (0 para el mes de alta, 1 para el siguiente, etc., como entero), `usuarios_cohorte` (tamaño total de la cohorte), `usuarios_activos` (cuántas personas **distintas** de esa cohorte tuvieron al menos una reproducción en ese mes, en UTC) y `retencion_pct` (porcentaje sobre el tamaño de la cohorte, redondeado a 2 decimales). Incluye todas las cohortes. Ordena por `cohorte` y luego por `mes_indice` ascendente.",
    learning_objective:
...
ORDER BY a.cohorte, 2;`,
  ...
  },
  {
    ...
    tables_used: ["users", "plays"],
    scenario_md:
      "La matriz completa es demasiado grande para la presentación al directorio. Analítica quiere una sola curva: el porcentaje promedio de oyentes que sigue activo en cada mes de vida, calculada solo sobre las cohortes que ya vivieron los seis primeros meses completos.",
    business_question_md:
      "Toma únicamente a los oyentes con `signup_at` anterior al `2025-04-01` (en UTC): son las cohortes de `2024-01` a `2025-03`, las únicas que completaron seis meses de vida dentro de los datos. Para cada `mes_indice` de 0 a 5 devuelve `mes_indice` (entero), `usuarios_base` (total de oyentes considerados, igual en todas las filas), `usuarios_activos` (personas distintas con al menos una reproducción en ese mes de vida, en UTC) y `retencion_pct` (redondeado a 2 decimales), ordenado por `mes_indice` ascendente.",
    learning_objective:
...
ORDER BY a.mes_indice;`,
  ...
  },
  {
    ...
    scenario_md:
      "Finanzas no mide retención de oyentes: mide suscripciones pagas. Quiere la serie mensual de churn de Ritmo para el período junio de 2024 – agosto de 2025, con la regla de siempre: cuántas suscripciones estaban vivas al empezar el mes y cuántas se dieron de baja durante ese mes.",
    business_question_md:
      "Usando `subscriptions`, devuelve una fila por mes desde `2024-06-01` hasta `2025-08-01` con: `mes` (primer día del mes, como `date`), `activas_inicio` (suscripciones con `started_on` anterior al mes y que no habían terminado antes de él, es decir `ended_on` nulo o mayor o igual al primer día del mes), `bajas` (suscripciones cuyo `ended_on` cae dentro de ese mes) y `churn_pct` (`bajas / activas_inicio` en porcentaje, redondeado a 2 decimales). Ordena por `mes` ascendente.",
    learning_objective:
...
ORDER BY m.mes;`,
  ...
  },
  {
    ...
    tables_used: ["users", "plays"],
    scenario_md:
      "Expansión quiere decidir dónde invertir el presupuesto de marketing del próximo trimestre. Antes de traer más gente necesita saber en qué mercados la gente se queda: la retención del mes 1, es decir, qué proporción de cada cohorte volvió a escuchar en el mes calendario siguiente al de su alta.",
    business_question_md:
      "Toma a los oyentes con `signup_at` anterior al `2025-08-01` (en UTC), para que el mes siguiente al alta esté completamente observado en todos los casos. Una persona está retenida en el mes 1 si tuvo al menos una reproducción en el mes calendario inmediatamente posterior al de su alta (ambos meses calculados en UTC). Devuelve `country`, `usuarios`, `retenidos_m1` y `retencion_m1_pct` (redondeado a 2 decimales), ordenado por `retencion_m1_pct` descendente y, en caso de empate, por `country` ascendente.",
    learning_objective:
