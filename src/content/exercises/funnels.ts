...
export const exercises: ExerciseDef[] = [
  {
    ...
    scenario_md:
      "En **Pídelo**, el departamento de Operaciones quiere por fin un embudo de conversión oficial del ciclo de vida del pedido. Hoy cada equipo cuenta sus propios números y ninguno coincide. Te piden la base: una sola tabla con los cinco pasos en orden y cuántos pedidos llegó a cada uno.",
    business_question_md:
      "Debes generar un dataset, a partir de la tabla `order_events`, con una fila por paso del embudo, con el número de paso bajo el encabezado `paso`, donde 1 corresponde al evento `'placed'`, 2 a `'accepted'`, 3 a `'preparing'`, 4 a `'picked_up'` y 5 a `'delivered'`, el nombre del evento tal como aparece en la tabla bajo el encabezado `etapa` y la cantidad de pedidos distintos con ese evento bajo el encabezado `pedidos`. El evento `'cancelled'` no es un paso del embudo, así que debes excluirlo. Ordena por `paso` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["order_events"],
    scenario_md:
      "El embudo del departamento de Operaciones ya está armado, pero en el comité alguien preguntó lo obvio: qué porcentaje representa cada paso. Te piden agregar las dos tasas que se usan siempre: cuánto conserva cada paso respecto del anterior y cuánto conserva respecto del primer paso.",
    business_question_md:
      "Debes generar un dataset sobre el embudo de la tabla `order_events`, con los pasos 1 para `'placed'`, 2 para `'accepted'`, 3 para `'preparing'`, 4 para `'picked_up'` y 5 para `'delivered'`, excluyendo el evento `'cancelled'`, que devuelva el `paso`, la `etapa`, la cantidad bajo el encabezado `pedidos`, el porcentaje de pedidos respecto del paso inmediatamente anterior, redondeado a 2 decimales, bajo el encabezado `pct_vs_anterior`, que queda en `NULL` en el primer paso, y el porcentaje respecto del paso 1, redondeado a 2 decimales, bajo el encabezado `pct_vs_inicio`. Ordena por `paso` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "**TiendaViva** no tiene una tabla de eventos: el ciclo de vida del pedido está repartido entre las tablas `orders`, `payments` y `shipments`. El departamento de Finanzas quiere el embudo del primer semestre de 2025 para cerrar el informe de medio año, y necesita que la base sean **todos** los pedidos creados y no solo los que pagaron. Te piden armar ese embudo.",
    business_question_md:
      "Debes generar un dataset que, para los pedidos creados entre el `'2025-01-01'` y el `'2025-06-30'` inclusive, en la zona horaria UTC, devuelva una fila por paso con el número de paso, del 1 al 4, bajo el encabezado `paso`, el nombre del paso, que puede ser `'creado'`, `'pagado'`, `'enviado'` o `'entregado'`, bajo el encabezado `etapa`, y la cantidad bajo el encabezado `pedidos`. Un pedido cuenta como **pagado** si tiene al menos un pago cuyo `status` es `'approved'` o `'refunded'`; cuenta como **enviado** si tiene un envío con valor en `shipped_at`; y cuenta como **entregado** si tiene un envío con valor en `delivered_at`. Ordena por `paso` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["orders", "payments", "shipments"],
    scenario_md:
      "El departamento de Producto de **TiendaViva** sospecha que la aplicación móvil convierte peor que la web y quiere el dato antes de pedir presupuesto para rehacer el proceso de compra. Te piden el embudo completo abierto por canal, sin recorte de fechas, es decir, sobre toda la historia disponible.",
    business_question_md:
      "Debes generar un dataset con una fila por canal, tomado de la columna `orders.channel`, con el `canal`, la cantidad de pedidos del canal bajo el encabezado `pedidos`, los que tienen al menos un pago cuyo `status` es `'approved'` o `'refunded'` bajo el encabezado `pagados`, los que tienen un envío con valor en `delivered_at` bajo el encabezado `entregados`, el porcentaje de `pagados` sobre `pedidos` con 2 decimales bajo el encabezado `pct_pago`, y el porcentaje de `entregados` sobre `pagados` con 2 decimales bajo el encabezado `pct_entrega_sobre_pago`. Ordena por `canal` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["order_events"],
    scenario_md:
      "Con el embudo sobre la mesa, la gerenta del departamento de Operaciones de **Pídelo** hace la pregunta que realmente importa: de los pedidos que no se entregan, en qué punto exacto se quedan. Quiere una tabla donde cada pedido aparezca una sola vez, clasificado por el último paso que alcanzó, y te pide que la armes.",
    business_question_md:
      "Debes generar un dataset, a partir de la tabla `order_events`, que clasifique cada pedido por el **número del paso más alto** que alcanzó, donde 1 corresponde a `'placed'`, 2 a `'accepted'`, 3 a `'preparing'`, 4 a `'picked_up'` y 5 a `'delivered'`, y donde el evento `'cancelled'` no hace avanzar el embudo. Devuelve el `ultimo_paso`, el nombre del evento correspondiente a ese paso bajo el encabezado `ultima_etapa`, la cantidad de pedidos cuyo último paso es ese bajo el encabezado `pedidos` y el porcentaje sobre el total de pedidos, con 2 decimales, bajo el encabezado `pct_del_total`. Ordena por `ultimo_paso` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Marketing de **Ritmo** quiere lanzar una campaña que empuje a los oyentes a armar su primera lista de reproducción, con la hipótesis de que armar una lista lleva a suscribirse. Antes de invertir, te piden medir el camino de activación. Sabes que hay dos maneras de contarlo y que dan números muy distintos, así que decides entregar las dos en la misma tabla.",
    business_question_md:
      "Debes generar un dataset con el embudo de activación en cuatro pasos: el 1 es `'alta'`, que incluye a todos los usuarios, el 2 es `'primera escucha'`, el 3 es `'primera playlist'` y el 4 es `'primera suscripcion'`. Devuelve el `paso`, la `etapa` y dos conteos: la cantidad de usuarios que tienen ese hito, sin importar cuándo, bajo el encabezado `usuarios_cualquier_orden`, y la cantidad de usuarios que tienen ese hito y todos los anteriores, cada uno **posterior** al anterior, bajo el encabezado `usuarios_orden_estricto`. Usa siempre el **primer** hito de cada tipo, con la función `min`, y compara en la zona horaria UTC; como la columna `subscriptions.started_on` es una fecha, cuenta la suscripción como posterior a la lista cuando ocurrió el mismo día o después. Ordena por `paso` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "El embudo de **Pídelo** convierte bien, pero el departamento de Operaciones sospecha que en algunas ciudades el pedido tarda demasiado en avanzar. Te piden los tiempos entre pasos por ciudad del restaurante, con una advertencia explícita: no quieren un promedio del tiempo de entrega, porque un par de pedidos trabados les distorsionó el último informe.",
    business_question_md:
      "Debes generar un dataset usando la tabla `order_events`. Arma primero una fila por pedido con el primer instante de `'placed'`, el de `'accepted'` y el de `'delivered'`; las tres son marcas de tiempo con huso horario, así que las diferencias entre ellas no dependen de la zona horaria de la sesión. Quédate solo con los pedidos que llegaron a entregarse y agrúpalos por la ciudad del restaurante. Devuelve el nombre de la ciudad, tomado de `cities.name`, bajo el encabezado `ciudad`, la cantidad bajo el encabezado `pedidos_entregados`, el promedio de minutos entre `'placed'` y `'accepted'`, con 2 decimales, bajo el encabezado `min_prom_aceptacion`, y la mediana de minutos entre `'placed'` y `'delivered'`, con 2 decimales, bajo el encabezado `min_mediana_entrega`. Ordena por `min_mediana_entrega` descendente y, si dos ciudades empatan, debes desempatar usando `ciudad` ascendente.",
    learning_objective:
