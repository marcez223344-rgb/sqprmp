...
export const exercises: ExerciseDef[] = [
  {
    ...
    scenario_md:
      "El departamento de Catálogo de **TiendaViva** quiere saber cuántos productos cuelgan de cada categoría final, la que ya no tiene subcategorías por debajo. Alguien escribió esto y lo dio por terminado:\n\n```sql\nSELECT c.id, c.name\nFROM categories AS c\nWHERE c.id NOT IN (SELECT parent_id FROM categories);\n```\n\nDevuelve **cero filas**, y en el catálogo hay 30 categorías de las cuales solo 6 tienen subcategorías. La consulta corrió sin errores.",
    business_question_md:
      "Debes generar un dataset que devuelva una fila por cada categoría que **no** sea padre de ninguna otra: `categoria_id` (el `id`), `categoria` (el `name`) y `productos` (cuántas filas de `products` tienen esa `category_id`; 0 si no tiene ninguna). Ordena por `categoria_id` ascendente.",
    learning_objective:
...
ORDER BY c.id;`,
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Marketing quiere saber, ciudad por ciudad, qué proporción de los clientes registrados llegó alguna vez a recibir un pedido. El informe que circula muestra **100 % en todas las ciudades**, lo que nadie cree. La consulta era esta:\n\n```sql\nSELECT c.country, c.city,\n  count(DISTINCT c.id) AS clientes,\n  count(DISTINCT o.customer_id) AS clientes_con_entrega\nFROM customers AS c\nLEFT JOIN orders AS o ON o.customer_id = c.id\nWHERE o.status = 'delivered'\nGROUP BY 1, 2;\n```",
    business_question_md:
...
ORDER BY c.country, c.city;`,
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Finanzas reporta 1 705 368 745,77 de ingresos entregados en Argentina. El tablero nuevo muestra 3 631 177 874,05 para el mismo período y el mismo filtro. Nadie tocó los datos. La consulta del tablero es esta:\n\n```sql\nSELECT c.country, count(*) AS pedidos, round(sum(o.total_amount), 2) AS ingresos\nFROM orders AS o\nINNER JOIN customers AS c ON c.id = o.customer_id\nINNER JOIN order_items AS oi ON oi.order_id = o.id\nWHERE o.status = 'delivered'\nGROUP BY 1;\n```\n\nEl join con `order_items` estaba ahí porque alguna versión anterior mostraba la cantidad de artículos.",
    business_question_md:
      "Debes generar un dataset que devuelva una fila por país con los pedidos entregados: `country`, `pedidos` (cantidad de pedidos con `status = 'delivered'`) e `ingresos` (suma de `total_amount` de esos pedidos, redondeada a 2 decimales). Cada pedido debe contarse y sumarse **una sola vez**. Ordena por `country` ascendente.",
    learning_objective:
...
ORDER BY country, channel;`,
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Operaciones negocia con los transportistas de **TiendaViva** y necesita el tiempo medio de entrega de cada uno. La versión que llegó a la mesa de negociación calcula el promedio dividiendo la suma de horas por la cantidad total de envíos, y da 121,38 horas para Andes Express. El sistema del transportista informa 131,11. La diferencia no es chica cuando hay un contrato de por medio.",
    business_question_md:
      "Debes generar un dataset, a partir de la tabla `shipments`, con una fila por transportista: `carrier`, `envios` (todos los envíos registrados), `entregados` (los que tienen `delivered_at`), `sin_entregar` (los que no lo tienen) y `horas_promedio` (promedio de horas entre `shipped_at` y `delivered_at`, **solo** sobre los entregados, redondeado a 2 decimales). Ordena por `carrier` ascendente.",
    learning_objective:
...
ORDER BY s.carrier;`,
  ...
  },
  {
    ...
    business_question_md:
      "Debes generar un dataset construyendo la cadena de filtros paso a paso, cada uno aplicado **sobre el resultado del paso anterior**: (1) todos los pedidos; (2) los que tienen `status` en `'delivered'` o `'returned'`; (3) los que además tienen al menos un pago con `status = 'approved'`; (4) los que además tienen al menos un envío con `delivered_at` no nulo; (5) los que además no tienen ninguna fila en `returns`. Devuelve `paso` (1 a 5), `etapa` (el texto exacto `pedidos creados`, `entregados o devueltos`, `con pago aprobado`, `con envio entregado`, `sin devolucion`), `pedidos` (cuántos sobreviven) y `perdidos` (cuántos perdió ese paso respecto del anterior; 0 en el paso 1). Ordena por `paso` ascendente.",
    learning_objective:
...
ORDER BY paso;`,
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Calidad quiere priorizar auditorías de vendedores por categoría y pidió la tasa de devolución de cada una. La primera versión une `orders`, `order_items`, `products` y `categories` y cuenta filas; el resultado ordena las categorías de una forma que no coincide con lo que Atención al Cliente ve todos los días, y los totales por categoría suman más pedidos de los que existen.",
    business_question_md:
      "Debes generar un dataset considerando solo los pedidos cuyo `status` es `'delivered'` o `'returned'`. Un pedido «toca» una categoría si alguna de sus líneas corresponde a un producto de esa categoría; un pedido puede tocar varias categorías y debe contarse **una sola vez en cada una**. Devuelve `categoria` (el `name` de `categories`), `pedidos` (pedidos distintos que tocan la categoría), `devueltos` (cuántos de ellos tienen `status = 'returned'`) y `tasa_pct` (porcentaje redondeado a 2 decimales). Ordena por `tasa_pct` descendente y, si hay empate debes desempatar usando `categoria` ascendente.",
    learning_objective:
