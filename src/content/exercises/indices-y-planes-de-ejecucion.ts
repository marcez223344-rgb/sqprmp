...
export const exercises: ExerciseDef[] = [
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "En **Pídelo**, alguien del equipo abrió un ticket pidiendo «un índice sobre `orders.status`, porque filtramos por estado todo el tiempo». Ingeniería de datos te devuelve la pregunta: antes de crear el índice quiere saber **cuántas filas deja pasar cada valor** de esa columna.\n\nEsa proporción se llama **selectividad** y se mide con SQL común, sin ver ningún plan de ejecución.",
    business_question_md:
      "Devuelve una fila por cada valor de `status` en `orders`, con `status`, la cantidad de pedidos como `pedidos` y el porcentaje que representa sobre el total de la tabla como `pct_del_total`, redondeado a **2 decimales**. Ordena de mayor a menor `pedidos`.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "Ingeniería de datos de **Pídelo** va a revisar los índices de `orders` y te pide una foto previa: cuántas filas tiene la tabla y **cuántos valores distintos** tiene cada columna por la que se filtra o se une.\n\nEsa cantidad de valores distintos se llama **cardinalidad**. Cuanto más alta, menos filas deja pasar en promedio un filtro de igualdad por esa columna, y más rinde un índice.",
    business_question_md:
      "Devuelve **una sola fila** con seis columnas, en este orden: `filas` (todas las filas de `orders`), `clientes_distintos`, `restaurantes_distintos`, `repartidores_distintos`, `estados_distintos` y `metodos_distintos`, cada una con la cantidad de valores distintos de `customer_id`, `restaurant_id`, `courier_id`, `status` y `payment_method`.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "El panel que usa cada restaurante de **Pídelo** para ver su mes tarda ocho segundos. La consulta que lo alimenta filtra `orders` por un restaurante y un mes. Vas a pedir un índice compuesto por `(restaurant_id, placed_at)`, y el pedido necesita el número que ingeniería siempre reclama: **cuántas filas deja pasar ese filtro y sobre cuántas filas de la tabla**.\n\nEl caso concreto es el restaurante con `id` 194, *Sabor Peralta 194* de Ciudad de México, durante agosto de 2025.",
    business_question_md:
      "Devuelve **una sola fila** con tres columnas: `filas_tabla` (todas las filas de `orders`), `filas_filtro` (las que cumplen `restaurant_id = 194` **y** `placed_at` dentro de agosto de 2025 en UTC) y `pct_del_total`, el porcentaje de la segunda sobre la primera redondeado a **4 decimales**.\n\nResuélvelo en **una sola pasada** por la tabla: sin `WHERE` que recorte las filas, porque necesitas el total y el filtrado en la misma fila de resultado.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "Ingeniería de datos creó el índice compuesto `(restaurant_id, placed_at)` sobre `orders`, es decir un índice ordenado primero por restaurante y, dentro de cada restaurante, por el momento del pedido.\n\nAhora el panel de *Sabor Peralta 194* (restaurante con `id` 194, en Ciudad de México) tiene que pedir su trimestre en la forma que ese índice puede servir: igualdad por el restaurante, rango por la fecha, y el resultado ya ordenado por fecha.",
    business_question_md:
      "Devuelve `id`, `placed_at`, `status` y `total` de los pedidos del restaurante `194` hechos entre el **1 de julio de 2025 inclusive y el 1 de octubre de 2025 exclusive, en UTC**, ordenados por `placed_at` ascendente y, ante empates, por `id` ascendente.\n\nEscribe el filtro de fecha como un rango semiabierto sobre `placed_at`. La práctica **no acepta** `date_trunc`, `extract` ni `to_char` aplicados a esa columna.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "El panel de *Sabor Peralta 194* agrega una tarjeta: cuántos pedidos recibió por mes desde que entró a **Pídelo**. Es la consulta más liviana que se le puede pedir a `orders`, porque las únicas columnas que necesita son `restaurant_id` y `placed_at`.\n\nEse detalle importa. Un índice que contiene **todas** las columnas que la consulta usa se llama **índice cubriente**, y le permite al motor responder sin tocar la tabla: en un plan aparece como `Index Only Scan`. Con `(restaurant_id, placed_at)` esta tarjeta está cubierta.",
    business_question_md:
      "Devuelve una fila por mes con actividad del restaurante `194`: `mes`, el primer día del mes de `placed_at` calculado **en UTC** y de tipo `date`, y `pedidos`, la cantidad de pedidos de ese mes. Ordena por `mes` ascendente.\n\nNo filtres por fecha: la tarjeta muestra toda la historia del restaurante.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "Operaciones de **Pídelo** quiere el cierre de agosto de Ciudad de México: cuánto entregó cada restaurante de la ciudad. Es la consulta del plan que viste en la lección, con tres tablas y una agrupación.\n\nEl motor la va a resolver con un **Hash Join**: arma en memoria una tabla de búsqueda con el lado chico, los restaurantes, y pasa por encima el lado grande, los pedidos ya recortados al mes. Tu parte es escribirla de forma que el lado grande llegue lo más chico posible.",
    business_question_md:
      "Para los pedidos con `status = 'delivered'` y `placed_at` en **agosto de 2025 en UTC**, de restaurantes cuya ciudad se llame `Ciudad de México`, devuelve una fila por restaurante con `restaurant_id` (el `id` del restaurante), `restaurante` (su `name`), `pedidos` (cantidad de pedidos entregados) y `ventas` (la suma de `total`, redondeada a **2 decimales**). Ordena por `ventas` descendente y, ante empates, por `restaurant_id` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "Ingeniería de datos de **Pídelo** abre la revisión trimestral de índices de `orders` y te pide una sola tabla para arrancar la reunión: los filtros que más corren en producción, cuántas filas deja pasar cada uno y un veredicto preliminar.\n\nLos cinco filtros que quiere medir son: el restaurante 194 en agosto de 2025, el restaurante 194 en toda su historia, agosto de 2025 para todos los restaurantes, `status = 'cancelled'` y `status = 'delivered'`.",
    business_question_md:
      "Devuelve **una fila por filtro**, con cinco columnas:\n\n- `filtro`: el nombre del filtro, exactamente uno de estos textos — `restaurante 194 en agosto de 2025`, `restaurante 194, toda la historia`, `agosto de 2025, todos los restaurantes`, `status = cancelled`, `status = delivered`.\n- `filas_que_pasan`: cuántas filas de `orders` cumplen ese filtro.\n- `filas_tabla`: el total de filas de `orders`, el mismo valor en las cinco filas.\n- `pct_del_total`: el porcentaje de la segunda columna sobre la tercera, redondeado a **4 decimales**.\n- `veredicto`: `indice muy conveniente` si el porcentaje es menor que 1, `indice probablemente util` si es menor que 10, e `indice poco util` en cualquier otro caso.\n\nLos dos filtros de agosto de 2025 son en **UTC**. Ordena por `pct_del_total` ascendente y, ante empates, por `filtro` ascendente.",
    learning_objective:
