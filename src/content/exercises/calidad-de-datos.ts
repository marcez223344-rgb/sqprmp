...
export const exercises: ExerciseDef[] = [
  {
    ...
    tables_used: ["plays"],
    scenario_md:
      "Es tu primer día en el departamento de Datos de **Ritmo**. Te dan acceso a `plays`, la tabla de reproducciones, y te piden un reporte de escucha «para mañana». Antes de calcular nada, necesitas saber con qué estás trabajando: cuánta información hay, qué período cubre y cuánto falta.",
    business_question_md:
      "Debes generar un dataset que devuelva **una sola fila** con el perfil de `plays`: `total_rows` (filas), `distinct_users` (oyentes distintos), `distinct_tracks` (canciones distintas), `first_played_at` y `last_played_at` (la primera y la última reproducción, **en UTC**), `seconds_played_nulls` (filas sin `seconds_played`) y `device_nulls` (filas sin `device`).",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["plays"],
    scenario_md:
      "El departamento de Producto de **Ritmo** quiere saber desde qué dispositivos se escucha música. La analista anterior entregó una tabla con cinco dispositivos que sumaban 107 637 reproducciones, pero la tabla tiene 109 382 filas. Faltan 1745 y nadie sabe dónde están.",
    business_question_md:
      "Debes generar un dataset que devuelva el reparto de reproducciones por dispositivo **sin perder ninguna fila**: `device` (el valor de `device`, y el texto `sin_dato` cuando está vacío), `plays` (cantidad de reproducciones) y `play_pct` (porcentaje sobre el total de la tabla, con dos decimales). Ordena de mayor a menor cantidad de reproducciones.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Contenidos de **Ritmo** paga regalías según los segundos escuchados de cada canción. Antes de firmar la liquidación del trimestre, finanzas te pide una cifra: cuántas filas de `plays` tienen un valor de `seconds_played` que no puede ser cierto.",
    business_question_md:
      "Debes generar un dataset que devuelva **una fila** con el resultado de los tres chequeos sobre `seconds_played`: `checked_rows` (filas evaluadas), `null_seconds` (sin valor), `negative_seconds` (menor que cero), `over_duration` (mayor que la duración de la canción, según `tracks.duration_seconds`) e `invalid_pct` (porcentaje de filas que fallan **alguno** de los tres chequeos, con dos decimales).",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["plays", "tracks"],
    scenario_md:
      "Le mostraste al departamento de Ingeniería que 210 reproducciones registran más segundos escuchados que la duración de la canción. La respuesta fue: «serán redondeos del reproductor». Necesitas evidencia concreta de que no lo son.",
    business_question_md:
      "Debes generar un dataset que liste los **20 casos más extremos**: `play_id` (identificador de la reproducción), `track_title` (título de la canción), `duration_seconds` (duración de la canción), `seconds_played` (segundos registrados) y `played_ratio` (segundos registrados dividido por la duración, con dos decimales). Ordena por `played_ratio` descendente y, si hay empate debes desempatar usando `play_id` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "En **Pídelo**, el departamento de Finanzas concilia cada mes lo facturado a los restaurantes contra los ítems vendidos. Este mes la conciliación no cierra: hay pedidos con importe que no tienen ninguna línea de detalle. Te piden el listado completo para reclamarle al equipo de la app.",
    business_question_md:
      "Debes generar un dataset que liste los pedidos que **no tienen ninguna fila** en `order_items`. Devuelve `order_id`, `status`, `placed_at_utc` (fecha y hora del pedido **en UTC**), `restaurant_name` (nombre del restaurante) y `order_total` (columna `total` del pedido). Ordena por `placed_at_utc` ascendente y, si hay empate debes desempatar usando `order_id` ascendente.",
    learning_objective:
