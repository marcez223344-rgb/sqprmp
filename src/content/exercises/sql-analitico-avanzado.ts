...
export const exercises: ExerciseDef[] = [
  ...
  },
  {
    ...
    scenario_md:
      "El equipo de alianzas de **Bolsillo** quiere renegociar acuerdos rubro por rubro y necesita saber en cuáles hay un comercio dominante y en cuáles el volumen está repartido. Piden el podio de cada rubro con su peso relativo.",
    business_question_md:
      "Considerando las transacciones con `status = 'completed'` y `created_at` en el primer semestre de 2025 (desde el 1 de enero inclusive hasta el 1 de julio exclusive, en UTC) que tengan comercio asociado, devuelve para cada rubro los tres comercios con más operaciones: `categoria`, `puesto` (1 a 3), `comercio`, `operaciones` y `participacion`, el porcentaje que representan esas operaciones sobre el total del rubro en el período, con 2 decimales. Desempata por nombre de comercio ascendente. Ordena por `categoria` y `puesto`.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "Operaciones de **Pídelo** sospecha que los pedidos se cancelan en momentos muy distintos del ciclo y quiere verlo sin inventar categorías: qué secuencias de estados existen realmente y cuánto pesa cada una.",
    business_question_md:
      "Con la tabla `order_events`, arma para cada pedido la secuencia de sus eventos ordenada por `event_at` (desempata por `id`) y devuelve una fila por secuencia distinta: `ruta` (los eventos unidos por ` > `), `pasos` (cuántos eventos tiene la secuencia), `pedidos` (cuántos pedidos la siguieron) y `porcentaje` sobre el total de pedidos, con 2 decimales. Ordena por `pedidos` descendente y, ante empates, por `ruta`.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "El equipo de producto de **Ritmo** arma la pantalla «Lo más escuchado del bimestre» y su API espera un documento por artista, no una grilla. Te piden el resultado listo para publicar, con los mismos números que usa el tablero interno.",
    business_question_md:
      "Considerando las reproducciones con `played_at` desde el 1 de julio de 2025 inclusive hasta el 1 de septiembre de 2025 exclusive (en UTC), toma los 10 artistas con más reproducciones en ese período (desempata por nombre de artista ascendente) y devuelve `artista` y `payload`: el texto de un objeto **jsonb** con las claves `pais` (país del artista), `genero`, `oyentes` (usuarios distintos que lo escucharon), `reproducciones` y `participacion` (porcentaje de sus reproducciones sobre el total del período **de todos los artistas**, con 2 decimales). Ordena por reproducciones descendente y, ante empates, por `artista`.",
    learning_objective:
