...
export const exercises: ExerciseDef[] = [
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "En **Pídelo**, el departamento de Operaciones revisa cada mes cuántos pedidos se entregaron. La consulta que venían usando filtraba con la expresión `date_trunc('month', placed_at)` y tardaba minutos en el servidor de producción, donde la tabla `orders` tiene cientos de millones de filas.\n\nTu jefa de área te pide reescribirla con un filtro de rango, que es el mismo pedido de negocio expresado de una forma que el índice sobre `placed_at` sí puede aprovechar.",
    business_question_md:
      "Debes generar un dataset de **una sola fila** con una columna llamada `pedidos_entregados`, que contenga cuántos pedidos cuyo `status` es igual al texto `'delivered'` fueron hechos, según la columna `placed_at`, durante **julio de 2025 en la zona horaria UTC**.\n\nEscribe el filtro de fecha como un **rango semiabierto** sobre `placed_at`, es decir, mayor o igual que el inicio de julio y menor que el inicio de agosto. La práctica **no acepta** las funciones `date_trunc`, `extract` ni `to_char` aplicadas a la columna de fecha.",
    learning_objective:
  ...
  },
  {
    ...
    tables_used: ["orders"],
    scenario_md:
      "El departamento de Atención al Cliente quiere revisar caso por caso las entregas más lentas de agosto de 2025 para entender qué falló, y te pidió los veinte peores pedidos.\n\nLa versión anterior del reporte era `SELECT * FROM orders ORDER BY delivered_at LIMIT 20`: traía todas las columnas y, peor todavía, la lista cambiaba entre ejecuciones porque el criterio de orden no desempataba.",
    business_question_md:
      "Debes generar un dataset que devuelva los **20 pedidos entregados**, es decir, aquellos cuyo `status` es igual al texto `'delivered'`, con `placed_at` en **agosto de 2025 en la zona horaria UTC**, que más tiempo tardaron entre `placed_at` y `delivered_at`.\n\nLas columnas son el `id` del pedido bajo el encabezado `order_id` y la diferencia entre `delivered_at` y `placed_at` expresada en minutos enteros bajo el encabezado `minutos_reales`. Ordena por `minutos_reales` descendente y, si dos pedidos empatan, debes desempatar usando `order_id` ascendente. Devuelve solamente esas dos columnas.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "El departamento Comercial de **Pídelo** quiere contactar a los restaurantes activos que **no** vendieron nada en agosto. Antes de eso necesitan la lista contraria como control: los activos que sí tuvieron al menos una entrega en el mes, y te piden que la obtengas.\n\nLa consulta que circula en el equipo une la tabla `restaurants` con `orders` y arregla las repeticiones con `DISTINCT`. Funciona, pero genera una fila por pedido, más de mil en total, para después descartarlas casi todas. Este ejercicio te pide la versión que nunca las genera.",
    business_question_md:
      "Debes generar un dataset que devuelva los restaurantes que tienen la columna `is_active` en `true` y **al menos un** pedido cuyo `status` es igual al texto `'delivered'` y cuya columna `placed_at` cae en **agosto de 2025 en la zona horaria UTC**.\n\nLas columnas son el `id` del restaurante bajo el encabezado `restaurant_id` y el `name`. Ordena por `restaurant_id` ascendente. La práctica **no acepta** la palabra clave `DISTINCT` en esta consulta.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Producto de **Pídelo** quiere saber cuánto pesan las bebidas y los acompañamientos frente a los platos principales en Bogotá, y te pide ese desglose. Es una consulta que cruza cuatro tablas y que, escrita de cualquier manera, une 35 589 líneas de pedido con 14 437 pedidos antes de descartar casi todo.\n\nEl análisis se limita a Bogotá a propósito: los precios de cada ciudad están expresados en su propia moneda, así que sumar montos de varios países daría un número sin sentido.",
    business_question_md:
      "Debes generar un dataset que, tomando los pedidos **entregados**, es decir, aquellos cuyo `status` es igual al texto `'delivered'`, de restaurantes de **Bogotá**, que es la ciudad con `cities.id` igual a `5` y a la que los restaurantes pertenecen por la columna `restaurants.city_id`, con `placed_at` en **agosto de 2025 en la zona horaria UTC**, devuelva por categoría de ítem del menú la `category`, la suma de `order_items.quantity` bajo el encabezado `unidades` y la suma de `quantity * unit_price`, expresada en pesos colombianos y redondeada a dos decimales, bajo el encabezado `monto`.\n\nOrdena por `unidades` descendente.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "La gerencia de **Pídelo** en Lima pidió un tablero por restaurante con tres números del último trimestre: pedidos entregados, unidades vendidas y calificaciones recibidas. Te piden armarlo de forma que los tres números sean correctos.\n\nEl primer intento del equipo unía la tabla `orders` con `order_items` y con `ratings` en una sola consulta. Los números salieron el doble de grandes que los del sistema de encuestas y nadie entendía por qué.",
    business_question_md:
      "Debes generar un dataset que, para los restaurantes de **Lima**, que son los que tienen `restaurants.city_id` igual a `7`, con al menos un pedido cuyo `status` es igual al texto `'delivered'` entre el **1 de julio y el 30 de septiembre de 2025 inclusive, en la zona horaria UTC**, devuelva el `restaurant_id`, el `name`, la cantidad de pedidos entregados del período bajo el encabezado `pedidos`, la suma de `order_items.quantity` de esos pedidos bajo el encabezado `unidades` y la cantidad de filas de la tabla `ratings` que corresponden a esos pedidos bajo el encabezado `calificaciones`, con valor `0` cuando no hay ninguna.\n\nOrdena por `pedidos` descendente y, si dos restaurantes empatan, debes desempatar usando `restaurant_id` ascendente.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "El departamento de Operaciones de **Pídelo** paga un mínimo mensual a cada repartidor activo, entregue o no. Quieren la lista de quienes figuran activos y no entregaron ni un pedido en agosto de 2025, para reactivarlos o darlos de baja, y te piden que la obtengas.\n\nUn compañero escribió la consulta con `NOT IN` y le devolvió **cero filas**. Está convencido de que no hay repartidores inactivos; el resultado correcto no es cero.",
    business_question_md:
      "Debes generar un dataset que devuelva los repartidores que tienen la columna `is_active` en `true` y que **no** tienen ningún pedido cuyo `status` es igual al texto `'delivered'` y cuya columna `placed_at` cae en **agosto de 2025 en la zona horaria UTC** asignado a ellos por la columna `orders.courier_id`.\n\nLas columnas son el `id` del repartidor bajo el encabezado `courier_id`, el `full_name` y el `vehicle`. Ordena por `courier_id` ascendente. La práctica **no acepta** el operador `NOT IN` en esta consulta.",
    learning_objective:
  ...
  },
  {
    ...
    scenario_md:
      "La dirección de **Pídelo** pide el tablero trimestral de **Ciudad de México**: los diez restaurantes con más entregas, con su ticket promedio y su porcentaje de entregas tardías. Es la consulta que corre cada mañana antes de la reunión, así que tiene que ser rápida y, sobre todo, devolver siempre lo mismo. Te piden escribirla con esas dos condiciones.\n\nUna entrega se considera tardía cuando la columna `delivered_at` es posterior a `placed_at` más los `promised_minutes` que se le prometieron al cliente.",
    business_question_md:
      "Debes generar un dataset que, tomando los pedidos **entregados**, es decir, aquellos cuyo `status` es igual al texto `'delivered'`, entre el **1 de julio y el 30 de septiembre de 2025 inclusive, en la zona horaria UTC**, de restaurantes de **Ciudad de México**, que son los que tienen `restaurants.city_id` igual a `3`, devuelva los **10 restaurantes con más pedidos entregados**.\n\nLas columnas son el `restaurant_id`, el `name`, la cantidad de pedidos bajo el encabezado `pedidos`, el promedio de `orders.total`, expresado en pesos mexicanos y redondeado a dos decimales, bajo el encabezado `ticket_promedio`, y el porcentaje de esos pedidos entregados después de `placed_at` más `promised_minutes`, redondeado a un decimal, bajo el encabezado `pct_tardios`.\n\nOrdena por `pedidos` descendente y, si dos restaurantes empatan, debes desempatar usando `restaurant_id` ascendente.",
    learning_objective:
