...
export const exercises: ExerciseDef[] = [
  {
    ...
    business_question_md:
      "Debes generar un dataset considerando todos los `order_items`, sin filtrar por estado del pedido (es el supuesto que te dieron). Suma las unidades (`quantity`) vendidas por producto, agrupa por categoría y devuelve los productos que ocupan el **segundo nivel de unidades vendidas** dentro de su categoría: es decir, aquellos cuyo total es el segundo valor distinto más alto de la categoría. Si varios productos comparten ese total, devuélvelos todos. Columnas: `categoria` (nombre de la categoría), `product_name` y `unidades`. Ordena por `categoria` y luego por `product_name`.",
    learning_objective:
...
ORDER BY c.name, v.product_name;`,
  ...
  },
  {
    ...
    business_question_md:
      "Debes generar un dataset que devuelva los repartidores con `is_active = true` que **no** estén asignados a ningún pedido con `placed_at` dentro de agosto de 2025 **en UTC** (desde el 2025-08-01 00:00 inclusive hasta el 2025-09-01 00:00 exclusive). Columnas: `courier_id`, `full_name`, `city` (nombre de la ciudad), `vehicle` y `started_at`. Ordena por `city`, `full_name` y `courier_id`.",
    learning_objective:
...
ORDER BY transacciones_marcadas DESC, m.name, m.id;`,
  ...
  },
  {
    ...
    business_question_md:
      "Debes generar un dataset normalizando el correo con `lower(trim(email))` y quédate con los correos normalizados que aparecen en más de un cliente. Para cada uno devuelve: `correo` (el normalizado), `cuentas` (cuántos clientes lo comparten), `id_a_conservar` (el `id` del cliente con `signup_at` más antiguo; si hay empate, el `id` menor) y `pedidos_totales` (la cantidad de pedidos de **todas** las cuentas del grupo, incluidas las que no tienen ninguno). Ordena por `cuentas` descendente y luego por `correo`.",
    learning_objective:
...
ORDER BY g.cuentas DESC, g.correo;`,
  ...
  },
  {
    ...
    business_question_md:
      "Debes generar un dataset tomando los días, en la zona horaria UTC, en que cada oyente tuvo al menos una reproducción y calcula sus rachas de días **consecutivos**. Devuelve solo los oyentes cuya racha más larga sea de 10 días o más, con: `user_id`, `country`, `dias_racha` (largo de la racha), `inicio` y `fin` (primer y último día de esa racha). Si un oyente tiene dos rachas de la misma longitud, informa la que empezó antes. Ordena por `dias_racha` descendente y luego por `user_id`.",
    learning_objective:
...
ORDER BY b.user_id, tercera_utc;`,
  ...
  },
  {
    ...
    business_question_md:
      "Debes generar un dataset considerando solo los pedidos cuyo `status` es igual al texto 'delivered', y define el GMV de cada restaurante como la suma de `total`. Ordena los restaurantes por GMV descendente (desempata por `restaurant_id` ascendente) y devuelve los que hacen falta para alcanzar el 50 % del GMV total: todos aquellos cuyo acumulado **antes** de sumarlos todavía no llegaba al 50 %. Columnas: `posicion` (1 para el de mayor GMV), `restaurant_name`, `gmv` y `share_acumulado_pct` (participación acumulada sobre el GMV total, en porcentaje con 2 decimales, contando ese restaurante). Ordena por `posicion`.",
    learning_objective:
