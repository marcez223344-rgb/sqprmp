...
const rel = file.replace(
  normalizePath(input.cwd || process.env.CLAUDE_PROJECT_DIR || "") + "/",
  "",
);
...
if (hits.length) {
  decision(
    EVENT,
    "deny",
    `Write blocked: content looks like it contains ${hits.join(", ")}. Use placeholders and .env.local.`,
  );
  process.exit(0);
}
...
if (/(^|\/)\.env(\.[^/]+)?$/.test(rel) && !/\.env\.example$/.test(rel)) {
  decision(
    EVENT,
    "ask",
    "Editing a .env secret file; the owner normally manages these. Confirm placeholders only.",
  );
  process.exit(0);
}
...
if (/(^|\/)package-lock\.json$/.test(rel)) {
  decision(
    EVENT,
    "deny",
    "package-lock.json is managed by npm; run npm install/uninstall instead.",
  );
  process.exit(0);
}
if (/^public\/datasets\/.*\.(tar\.gz|tgz)$/.test(rel)) {
  decision(
    EVENT,
    "deny",
    "Dataset snapshots are built with `npm run datasets:build`, not written by hand.",
  );
  process.exit(0);
}
...
if (/^supabase\/migrations\/.+\.sql$/.test(rel)) {
  ...
  if (isEdit) {
    decision(
      EVENT,
      "ask",
      "Editing an existing migration. If it was already applied anywhere, create a new migration instead (rules/database.md).",
    );
    process.exit(0);
  }
}
...
if (/^\.claude\/(settings\.json|hooks\/.+)$/.test(rel)) {
  decision(
    EVENT,
    "ask",
    "Changing Claude Code hooks/settings; confirm with the owner and document in docs/CLAUDE_CODE_SETUP.md.",
  );
  process.exit(0);
}
